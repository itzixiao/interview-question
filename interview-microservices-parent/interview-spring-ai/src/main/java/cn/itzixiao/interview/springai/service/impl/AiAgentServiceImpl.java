...
public class AiAgentServiceImpl implements AiAgentService {
    ...
    public Flux<ChatResponse> chatStream(ChatRequest request) {
        ...
        // 流式调用 - Spring AI 1.0.0-M4 使用 stream 方法
        // 注意：流式响应需要特定的 ChatClient 配置
        return Flux.just(ChatResponse.builder()
                .content("流式响应需要配置 StreamChatClient，当前使用简化实现")
                .sessionId(sessionId)
                .success(true)
                .build())
                .onErrorResume(e -> {
                    log.error("Stream chat error: {}", e.getMessage(), e);
                    return Flux.just(ChatResponse.failure("流式聊天服务异常: " + e.getMessage()));
                });
    }
    ...
    public Flux<AgentTaskResponse> executeTaskStream(AgentTaskRequest request) {
        ...
        // 流式调用 - Spring AI 1.0.0-M4 使用 stream 方法
        // 注意：流式响应需要特定的 ChatClient 配置
        return Flux.just(new AgentTaskResponse(
                taskId,
                AgentTaskResponse.ExecutionStatus.SUCCESS,
                "流式响应需要配置 StreamChatClient，当前使用简化实现",
                null,
                null,
                System.currentTimeMillis() - startTime,
                null
        )).onErrorResume(e -> {
            log.error("Stream task execution error: {}", e.getMessage(), e);
            return Flux.just(new AgentTaskResponse(
                    taskId,
                    AgentTaskResponse.ExecutionStatus.FAILED,
                    null,
        ...
        if (request.getHistory() != null) {
            for (ChatRequest.MessageHistory history : request.getHistory()) {
                switch (history.getRole()) {
                    ...
                    case "assistant" -> messages.add(new org.springframework.ai.chat.messages.AssistantMessage(history.getContent()));
