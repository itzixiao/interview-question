...
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='设备运行日志（大数据量导出测试表）';
...
BEGIN
    ...
    WHILE i <= num_records DO
        ...
        VALUES 
        (v_device_code, v_device_name, v_operation_type, v_operation_value, v_operation_time, v_operator, CONCAT('测试数据-', i));
...
-- 4. 验证数据
SELECT COUNT(*) AS total_records FROM device_operation_log;

-- 查看前 10 条数据
SELECT * FROM device_operation_log LIMIT 10;
...
-- 查看时间范围
SELECT 
    MIN(operation_time) AS earliest_time,
...
-- 测试全表扫描
EXPLAIN SELECT * FROM device_operation_log;
