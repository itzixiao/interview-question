...
- [一、RPC 基础概念](#一rpc-基础概念)
- [二、RPC 架构详解](#二rpc-架构详解)
- [三、RPC 实现原理](#三rpc-实现原理)
...
**RPC（Remote Procedure Call，远程过程调用）** 是一种通过网络从远程计算机程序上请求服务的软件协议。它的核心目标是*
*让本地程序像调用本地方法一样调用远程服务**，对调用者屏蔽底层网络通信细节。
