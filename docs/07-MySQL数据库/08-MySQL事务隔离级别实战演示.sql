...
-- 插入初始数据
INSERT INTO test_account VALUES (1, '张三', 1000.00);
INSERT INTO test_account VALUES (2, '李四', 1000.00);
...
-- 验证可重复读
START TRANSACTION;
SELECT * FROM test_account WHERE id = 1; -- 600.00
COMMIT;
...
-- 重置数据
UPDATE test_account SET balance = 1000.00 WHERE id IN (1, 2);
DELETE FROM test_account WHERE id = 3;
...
-- 尝试插入 id=3 的记录
INSERT INTO test_account VALUES (3, '赵六', 1000.00);
-- 【现象】可能被阻塞（Next-Key Lock 防止幻读）

-- 或者尝试插入 id=4
INSERT INTO test_account VALUES (4, '钱七', 1000.00);
-- 【现象】也可能被阻塞
...
INSERT INTO inventory VALUES (1, 'iPhone 15', 10);
...
-- ============ 会话 A ============
-- 连续扣减 8 次
UPDATE inventory SET stock = stock - 1 WHERE product_id = 1; -- 7 次...
COMMIT;
