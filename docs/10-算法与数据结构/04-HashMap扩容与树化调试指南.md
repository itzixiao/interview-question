...
插入第 13 个元素时：
  → 先将第 13 个元素插入到当前数组（容量 16）
  → size 增加到 13
  → 检查 size(13) > threshold(12) 条件满足
  → 调用 resize() 扩容到 32（将已插入的 13 个元素迁移到新数组）
```

**注意：** 扩容是在元素插入**之后**触发的，然后将已插入的所有元素重新哈希到新数组。
...
**结论：** 当 `size > threshold` 时，在当前元素插入完成后触发扩容，然后将所有元素迁移到新数组。
...
**答案：**
扩容是在元素成功插入后才触发的，原因如下：

1. **简化逻辑**：先确保元素插入成功，再处理扩容
2. **避免重复计算**：如果先扩容再插入，插入失败时需要回滚扩容操作
3. **源码设计**：`resize()` 在 `putVal()` 的最后执行，此时元素已插入完成
...
++modCount;
// 元素已经插入完成（前面代码已完成插入）
if (++size > threshold)  // size 增加后检查是否超过阈值
    resize();            // 超过则扩容，迁移所有元素
afterNodeInsertion(evict);
```

**注意：** 扩容时会创建新数组，并将已插入的所有元素重新哈希到新数组中。
...
| 操作 | 时机      | 触发条件                              | 执行顺序        |
|----|---------|-----------------------------------|-------------|
| 扩容 | 插入完成后   | `size > threshold`                | **先插入，再扩容** |
| 树化 | 插入链表节点后 | `binCount >= 8 && capacity >= 64` | **先插入，再转树** |

**记忆口诀：**

- 扩容看大小（size > threshold），插后再扩
